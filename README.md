<p align="center">
 
<img src='https://user-images.githubusercontent.com/42920754/124822528-5fa0a380-df46-11eb-90e7-d680cc02dda7.jpg' width='100%' heigth="10%">
<p/>



# Calculadora 

<p align='Center'>
Clique aqui e baixe na google play!
<p/>

<p align='Center'>
 :point_down:
<p/>
 
<p align='Center'>
<a href="https://play.google.com/store/apps/details?id=gusoliveira21.calculadora">
<img src='https://user-images.githubusercontent.com/42920754/125994119-9b6389fc-ba26-49d7-bd53-69bcd02b2045.jpg' width='15%'>
</a>
<p/>


<p align='Center'>
:point_up:
<p/>
<br>

## :star: Objetivo:

<p align='Justify'> 
Este trabalho teve como objetivo a construção de uma calculadora simples, que teve como ponto principal a o estudo e prática da linguagem Kotlin no desenvolvimento Android Nativo o a resolução de bugs encontrados em outras ferramentas similares, como:
<p/>

* A possibilidade de digitar expressões matemáticas repetidas vezes, como:

 ~~~ 
 +++ , --- , /// e *** 
 ~~~

* A possibilidade de digitar pontos/virgulas repetidas vezes. como:

 ~~~ 
 ... , 8.2.3.6 
 ~~~
 
 * A possibilidade de digitar pontos/virgulas no início e fim de cada operação. como:
 
 ~~~ 
 .8+5+6 , 82.3+.6 
 ~~~

## :exclamation: Exemplo:

<p align="center">
<img src='https://user-images.githubusercontent.com/42920754/124821615-506d2600-df45-11eb-8188-0b3d0f1d8da3.gif' width='15%'>
<p/>


## :green_book: Tarefas realizadas:
- [x] Criação de uma calculadora básica.
- [x] Impossibilitar digitação de expressões matemáticas seguidas vezes.
- [x] Impossibilitar a digitação de pontos/virgulas seguidas vezes.
- [x] Impossibilitar a escrita de pontos/virgulas no início e fim de cada operação.
- [x] Limitar operações em duas casas decimais.- 
- [x] Dependendo da saída, verificar se deve ter casas decimais.
- [x] Não permitir o app a girar com a tela.
- [x] Mudança no design da tela.
- [x] Adicionar ao modelo MVVM utilizando ferramentar do Jetpack.
- [x] Adicionr layout horizontal.

<br>
<p align="center">
<img src='https://user-images.githubusercontent.com/42920754/124818345-4ea16380-df41-11eb-9c0b-19415296d8ba.gif' width='20%'>
___
<img src='https://user-images.githubusercontent.com/42920754/125992072-af1d5ccc-bd5c-49ac-a8d1-6daac50625a9.png' width='20%'>

<p/>
<br>



## :pushpin: Tarefas em aberto:
- [ ] Usar vinculação de dados em conjunto com o ViewModel 

<br><br><br><br>

## ☕ Donation

Se este projeto lhe ajudou de alguma forma não esqueça de contribuir para o café desse dev! :)

[![paypal](https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif)](https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=P87GHQLSDSJU2&source=url)
